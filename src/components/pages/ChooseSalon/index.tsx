...
const SalonChoice = () => {
  ...
  const getCoordinates = (salons) => {
    ...
    salons.forEach((fsalon) => {
      if (fsalon.address.lat && fsalon.address.long) {
        positionArray.push({
          lat: Number(fsalon.address.lat),
          lng: Number(fsalon.address.long),
        });
      }
    });
  ...
  }, [isLoading, filteredSalons.length, isLoggedIn]);

  // Fonction pour fermer le modal
  const closeModalCustomerInfo = () => {
    if (!isLoggedIn) setIsCustomerInfoModalOpen(false);
  };
  ...
  const handleAllFilter = async () => {
    ...
    const param = {
      ...
      countryFilter:
        countryFilter && countryFilter !== "null" ? countryFilter : "",
  ...
  const onWishlist = async (e: any, salon: SalonDetails) => {
    ...
    if (userId) {
      ...
      if (salon.wishlist) {
        await dashboard
          .removeFromSalonWishList(salon.id, userId)
          .then(() => {
            handleAllFilter();
  ...
  const getRedSVGWithValue = (value: string) => {
    const element = ReactDOMServer.renderToStaticMarkup(
      <MapIconRedWithValue value={value} />
    );
    return URL.createObjectURL(new Blob([element], { type: "image/svg+xml" }));
  };
  ...
  return (
    <>
      ...
      <TourModal
        ...
        doneTour={pageDone.includes("choose_salon")}
      />
      ...
      <div className="w-full h-screen  overflow-hidden">
          ...
          <BaseModal close={closeModalCustomerInfo} opacity={20}>
            <div className="text-center">
              <h2 className="text-3xl font-bold mb-4 text-gradient">
                Onehaircut est en plein essor !
              </h2>
              <p className="mb-8">
                Il y a moins de 10 salons qui correspondent à vos critères.
                <br />
                Nous travaillons activement pour ajouter plus de salons.
                <br />
              </p>
              ...
              <div className="flex justify-center items-center space-x-4 mt-8">
                ...
                {isLoggedIn && (
                  <button
                    ...
                    } text-white font-bold py-2 px-4 rounded`} // Changer la classe en fonction de l'état du bouton
        ...
        <Navbar
          ...
          onAvailabilityFilter={(availability: string[]) =>
        ...
        <div className="w-full flex-col items-center justify-center px-6">
          ...
          <p className="text-md lg:text-4xl font-medium text-black text-center md:mt-6">
            ...
            {filteredSalons.length === 1 ? "correspond" : "correspondent"} à vos
            critères
          </p>
          ...
          <div className="w-full flex items-end justify-between mt-2 md:mt-4 px-4">
            ...
            <div
              className="flex items-center cursor-pointer md:mt-4 mb-2 sm:mx-10 2xl:mx-14 text-stone-800"
              onClick={() => router.push("/services")}
            >
              <BackArrow />
              <p
                className={`text-xs md:text-xl text-stone-600 justify-center font-medium`}
              >
                Retour aux services
              </p>
            </div>
            ...
            <button
              ...
              onClick={onContinue}
              className={`flex items-center justify-center text-lg text-white font-medium w-full md:w-52 h-10 md:h-14 rounded-xl px-4 button_continue ${
                selectedSalon.id
                  ? Theme_A.button.medLargeGradientButton
                  : "bg-[#D9D9D9]"
              }`}
            >
              Continuer
          ...
          {isLoaded && positions.length > 0 && (
            <div className="w-full lg:h-screen mt-4  grid grid-rows-1 lg:grid-cols-2 gap-0 lg:gap-3 content-start ">
                          ...
                          <React.Fragment key={index}>
                            <MarkerF
                              ...
                              position={{
                                lat: position.lat,
                                lng: position.lng,
                              }} // Utiliser la position du salon
                              onClick={() =>
                                setSelectedSalon(
                                  filteredSalons[index] != null
                              ...
                              options={{
                                icon: {
                                  url:
                                    filteredSalons[index]?.id ===
                                    selectedSalon.id
                                      ? getRedSVGWithValue(
                                          `${convertAmount(
                                            filteredSalons[index]["user"][
                                              "currency"
                                            ],
                                            userCurrency,
                                  ...
                                  scaledSize:
                                    filteredSalons[index]?.id ===
                                    selectedSalon.id
                                      ? new window.google.maps.Size(70, 110)
                                      : new window.google.maps.Size(60, 100),
                                  origin: new window.google.maps.Point(0, -10),
                                  anchor:
                                    filteredSalons[index]?.id ===
                                    selectedSalon.id
                                      ? new window.google.maps.Point(25, 37)
                                      : new window.google.maps.Point(20, 35),
                                },
                                zIndex: position.zIndex
                                  ? position.zIndex + 1 + index
                                  : 1 + index,
                              }}
                              zIndex={
                                position.zIndex
              ...
              <div
                className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-6 pb-52 lg:pb-36 overflow-y-scroll content-start overflow-x-hidden thumbnails_salons"
                style={{ maxHeight: getHeightThumbnails() }}
              >
                {/* VIGNETTES (ITERATIONS) */}
                {filteredSalons.length > 0 &&
                  filteredSalons.map((fsalon, index) => {
                    return (
                      <div
                        key={index}
                        id={`Vignette-${index}`}
                        ...
                        className={`relative flex w-full w-max[450px] h-56 h-max[300px] rounded-2xl border hover:border-stone-400 cursor-pointer
                                        ${
                                          selectedSalon.id === fsalon.id
                                            ? "border-4 border-red-400 shadow-xl"
                                            : ""
                                        }
                                        ${
                                          fsalon.wishlist
                                            ? ColorsThemeA.OhcGradient_G
                                            : "bg-stone-100"
                                        }`} // bg-green-100 est un exemple, choisissez la couleur que vous voulez
                      >
                        {selectedSalon.id === fsalon.id && (
                        ...
                        <div
                          className="flex flex-col p-1 md:p-2 shadow-md rounded-2xl "
                          style={{ flexGrow: 1 }}
                        >
                          <div
                            className="relative mb-1 md:mb-4 hover:scale-105 transition duration-1000 m-2"
                            style={{
                              flexGrow: 1,
                              display: "flex",
                              alignItems: "center",
                          ...
                          >
                            {!isLoggedIn && (
                              <div
                                onClick={(e) => onWishlist(e, fsalon)}
                                className="absolute right-6 sm:right-2 top-6 sm:top-2 z-10 cursor-pointer hover:scale-150 transition duration-300"
                              >
                                <StarIcon
                                  width="35"
                                  height="35"
                            ...
                            {fsalon && fsalon.salon_cover_image && (
                              <Image
                                src={
                                  fsalon && fsalon.salon_cover_image
                                    ? fsalon.salon_cover_image?.image?.includes(
                                        "api"
                                      ) ||
                                      fsalon.salon_cover_image?.image?.includes(
                                        "127.0.0.1"
                          ...
                          <p className="text-black text-md md:text-lg font-semibold md:pt-2 overflow-auto">
                            {fsalon.name}
                          </p>
                          ...
                          <div className="flex items-center text-xs text-[#7B7B7B] pr-1 pt-1 gap-1">
                            <StarRatings
                              rating={
                                fsalon.salon_haircut
                                  ? fsalon.salon_haircut.rating
                                  : 0
                              }
                            ...
                            <p>
                              {fsalon.salon_haircut
                                ? fsalon.salon_haircut.rating_counts
                                : 0}{" "}
                              avis
                            </p>
                          </div>
