...
const settingsMenu: settingsStruct[] = [
  ...
  { name: "Horaires", permission: "Horaires", display: OpenningHours },
  {
    ...
    permission: "Indisponibilités",
    display: Unavailability,
  },
  {
    name: "Réglage des roles",
    permission: "Réglage des roles",
    display: RolesSettings,
  },
  {
    name: "Paiements",
    permission: "Paiements",
    display: PayementSettings,
    showWarning: !user_data?.bank_acc_stripe_id,
  },
...
const Settings = () => {
  ...
  const [activeMenu, setActiveMenu] = useState(
    settingsMenu[Object.keys(settingsMenu)[0]].name
  );
  ...
  const applyPermissions = (menus: any) => {
    ...
    if (user.role != "salon_professional" && user.permissions.length > 0) {
      menus.forEach((m: any, k: number) => {
        if (
          user.permissions.indexOf(m.permission || m.name) != -1 ||
          m.name == "Mot de passe"
        ) {
            ...
            {currentMenu.map((item, index) => {
              return (
                <div key={index} className="relative">
                  <p
                    className={` cursor-pointer md:ml-2 md:mr-2 ${
                      activeMenu === item.name && " text-black "
                    }`}
                  ...
                  >
                    ...
                    {item.showWarning && (
                      <>
                        <span className="absolute transform top-1/2 right-6 translate-x-1/2 -translate-y-1/2 h-3 w-3">
