...
const SlotDropdown = ({selectedItem, disabled, backgroundClr }: SlotDropdown) => {
  return (
    <div>
      <div className={`relative flex items-center px-3 border w-36 h-11 rounded-md ${` ${disabled ? 'bg-[rgba(171,171,171,0.10)] cursor-default' : backgroundClr ? `${backgroundClr} cursor-pointer` : 'bg-white cursor-pointer'}`}`}>
