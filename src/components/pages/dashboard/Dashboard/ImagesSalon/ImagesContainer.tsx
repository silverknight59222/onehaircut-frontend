...
const ImagesContainer = ({ title, type, salonImages, setIsLoading, getAllSalonImages }: ImagesContainerProps) => {
	...
	const addImage = async () => {
		...
		setIsLoading(true);
		await dashboard.addSalonImage(formData).then((res) => {
			getAllSalonImages();
			setSelectedImage("");
	...
	const deleteImage = async () => {
		if (updateMode) {
			setIsLoading(true);
			await dashboard.deleteSalonImage(updateMode.id).then((resp) => {
				getAllSalonImages();
				setUpdateMode(null);
				...
				<div className="flex justify-center">
					<div className="flex flex-col w-max gap-4">
						<div className="flex flex-col">
							<div className={`flex items-center p-4 rounded-2xl border-2 bg-white shadow-lg cursor-pointer`} onClick={handleClick}>
								<div className={`w-32 h-32 relative flex items-center`}>
									{updateMode || selectedImage ? (
										<Image
											src={updateMode ? updateMode.image : selectedImage}
											fill={true}
											alt={type === 'showcase' ? 'Showcase Image' : 'Hairstyle Image'}
										/>
									) : (
										<div>
											<p className="text-[10px] font-medium text-[#959595]">Photo</p>
							...
							<div>
								{validationErrors.image && !selectedImage && <p className="text-xs text-red-700 ml-1 mt-1">{validationErrors.image}*</p>}
							</div>
						</div>
						<div className="flex justify-end gap-2">
							...
							{!updateMode && <button onClick={addImage} className="flex items-center justify-center px-4 py-2 gap-4 rounded-md bg-gradient-to-r from-primaryGradientFrom via-primaryGradientVia to-primaryGradientTo shadow-[0px_14px_24px_0px_rgba(255,125,60,0.25)]">
								<AddIcon />
							</button>}
						</div>
					</div>
						...
						{salonImages.filter((item) => {
							...
							return (
								<div key={index} className="flex justify-center">
									<div onClick={() => selectImage(item)} className={`p-4 shadow-lg h-max flex flex-col justify-between cursor-pointer border-2 transition rounded-xl hover:border-secondary ${item.id === updateMode?.id && 'border-secondary'}`}>
