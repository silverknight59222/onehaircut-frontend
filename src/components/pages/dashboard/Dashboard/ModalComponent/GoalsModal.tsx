...
const StaffModal = () => {
    ...
    const revenueCards = [
        ...
        { title: 'Nouveaux clients', objective: '500.000 €' },
        { title: 'Nombre de visites en ligne', objective: '1.200 €' },
    ];

    const [progressBars, setProgressBars] = useState([
        { value: 61, name: "Revenu Mensuel", number: 27, color: "#FE2569, #FD4C55, #FF8636" },
        { value: 73, name: "Commandesd’ habitué", number: 47, color: "#0FBFF1, #4487F1" },
        { value: 50, name: "Nouveaux clients", number: 31, color: "#7ABF50, #418419" },
        { value: 0, name: "Nombre de visites en ligne", number: 0, color: "#15BAF2" },
    ]);

    const handleCardClick = (card: { title: string; objective: string }) => {
        ...
        <div>
            ...
            <div className="flex flex-row flex-wrap justify-center items-center gap-4">
                {revenueCards.map((card, index) => (
                    <div
                        style={{ width: '230px', height: '173px', borderRadius: '37px', background: '#E5E5E5' }}
                        key={index}
                        className="max-w-xs w-full rounded-xl shadow-md p-6"
                        onClick={() => handleCardClick(card)}
                    >
                        <h3 className="text-gray-900 text-xl font-medium mb-2">{card.title}</h3>
                        <p className="text-gray-500 text-sm mb-3">Objectif</p>
                        <p className="text-lg font-semibold text-gray-900 bg-white">{card.objective}</p>
                    </div>
                ))}
            </div>
        </div>
