...
const TextToDsplayifNoPic = (
  <div>
    <p className={`${Theme_A.textFont.infoTextSmall}`}>Ajouter une photo</p>
...
const DefaultProfilFace = (
  <Image
    src="/assets/PortraitClient/ProfilFace.png"
    alt=""
    fill={true}
    className="rounded-3xl "
  />
);
const DefaultProfilLeft = (
  <Image
    src="/assets/PortraitClient/ProfilLeft.png"
    alt=""
    fill={true}
    className="rounded-3xl "
  />
);
const DefaultProfilLeft2 = (
  <Image
    src="/assets/PortraitClient/ProfilLeft2.png"
    alt=""
    fill={true}
    className="rounded-3xl "
  />
);
const DefaultProfilRight = (
  <Image
    src="/assets/PortraitClient/ProfilRight.png"
    alt=""
    fill={true}
    className="rounded-3xl "
  />
);
const DefaultProfilRight2 = (
  <Image
    src="/assets/PortraitClient/ProfilRight2.png"
    alt=""
    fill={true}
    className="rounded-3xl "
  />
...
const Portrait = () => {
  // filters
  const Gender = ["Femme", "Homme", "Autre"];
  const Ethnicity = ["Afro", "Asian", "Occidental", "Oriental"];
  const Length = ["Court", "Moyen", "Long"];
  ...
  const [profileSlightlyLeftImage, setprofileSlightlyLeftImage] = useState<
    string | null
  >("");
  // handle the profil pic change
  ...
  const [profileSlightlyRightImage, setProfileSlightlyRightImage] = useState<
    string | null
  >("");
  // handle the profil pic change
  ...
  const profilPicToDisplay = (
    clickFct: () => void,
    img: string | null,
    subtitle: string,
    picSize: number,
    defaultPic: React.JSX.Element
  ) => {
    return (
      <div
        className=" flex flex-col gap-2 items-center transform hover:scale-105 transition-transform "
        onClick={clickFct}
        role="button"
        tabIndex={0}
      >
        <div className="relative p-4 rounded-2xl border-2 bg-white shadow-md hover:shadow-xl pic_left_profil">
          <div
            className={`w-${picSize} h-${picSize} relative flex text-center items-center`}
          >
            {img ? (
  ...
  const savePotraits = async () => {
    ...
    if (profileLeftImage) formData.append("left_profile", profileLeftImage);
    if (profileImage) formData.append("front_profile", profileImage);
    if (profileSlightlyRightImage)
      formData.append("slightly_right_profile", profileSlightlyRightImage);
    if (profileRightImage) formData.append("right_profile", profileRightImage);
    imagesToUpload.forEach((image) => {
      if (image) formData.append(image.type, image.file);
    });
  ...
  const tourSteps: Steps[] = [
    {
      ...
      content: (
        <div key="/assets/audio/tour/client/Portrait_woman_1.mp3">
          ...
          <p>
            Tu peux entrer ici tes photos de profil qui seront utilisées lors de
            la génération d'une coiffure sur ta tête.
          </p>
        </div>
      ),
    },
  ...
  return (
    <div>
      ...
      <TourModal
        ...
        doneTour={pageDone.includes("portrait")}
      />
      ...
      <div className="relative mt-10 mb-5 px-4 sm:px-6 md:px-20">
        <div className="flex flex-row items-center justify-center pb-10 w-full">
          ...
          {/* Info icon  */}
          <InfoButton
            title_1={"Photos de profil"}
            content_1={"Vous pouvez  disposer ici vos photos de profil."}
            content_2="Celles-ci seront utilisées lors de la génération de votre photos avec une coiffure voulue."
            onOpenModal={undefined}
          />
        </div>

        <p className="text-stone-400 font-normal italic text-sm text-center my-10">
          Pour modifiez une photo, cliquer sur celle-ci et sélectionner la
          remplaçante
        </p>
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start justify-center gap-10 mt-10"></div>
        <p className="text-stone-400 font-normal italic text-sm text-center my-10">
          Indiquer votre genre et groupe ethnique. Ceux-ci serviront lors de la
          presentation des coiffures à la page d'accueil
        </p>
