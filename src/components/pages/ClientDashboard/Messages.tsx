...
const Messages = () => {
  ...
  const getSalonsByUser = async () => {
    if (userId) {
      await clientDashboard
        .getSalonsByUser(userId)
        .then((resp) => {
  ...
  return (
    <div>
      ...
      <TourModal
        ...
        doneTour={pageDone.includes("message")}
      />
              ...
              return (
                salon && (
                  <div
                    ...
                    className={`flex items-center justify-between py-4 px-5 hover:bg-[#F5F5F5] mb-5 rounded-3xl cursor-pointer ${
                      selectedChat.user_id === salon.id &&
                      "bg-[#F5F5F5] outline outline-1 outline-red-200"
                    }`}
                  >
